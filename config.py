...
# ВАЖНО: здесь только ГЛОБАЛЬНЫЕ ПРАВИЛА (язык, формат, безопасность), а НЕ роль/персона.
# Роль и стиль задаёт пользователь через /prompt — и они перекрывают роль из этого
# блока, но не отменяют эти глобальные правила и guard-инструкцию.
