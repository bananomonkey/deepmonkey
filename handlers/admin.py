...
async def _build_persona_block(chat_id: int, target_id: int) -> tuple[bool, str]:
    ...
    try:
        ...
        block = (
            "ТЫ — НЕ БОТ И НЕ АССИСТЕНТ. ТЫ — ЧЕЛОВЕК ПО ИМЕНИ «{name}» ИЗ ЭТОГО ЧАТА. "
            "Забудь всю роль помощника. Общайся ТОЧНО как ведёт себя {name}: "
            "копируй его манеру речи, лексику, сленг, опечатки, тон, юмор, остроумие, "
            "привычки, отношение к людям, его типичные фразы и разговорный стиль. "
            "Отвечай так, будто ты и есть {name}. Не добавляй вежливых ассистентских "
            "формулировок, не поясняй, что ты ИИ. Пиши так, как написал бы он — "
            "кратко или длинно, ровно как он. Ниже его реальные сообщения — "
            "подражай именно им словом в слово по стилю.\n\n"
            f"### Человек: {name} (user_id {target_id})\n"
            f"### Описание его характера:\n{description}\n"
